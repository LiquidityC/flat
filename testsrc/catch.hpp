...
namespace Catch {
	...
	class StringRef
	{
	  ...
	  public: // substrings and searches
		auto substr(size_type start, size_type size) const noexcept
		  -> StringRef;
		...
		namespace Vector {
			...
			struct UnorderedEqualsMatcher : MatcherBase<std::vector<T>>
			{
				...
				bool match(std::vector<T> const& vec) const override
				{
					...
					//       because I don't want to include <algorithm> inside
					//       the common path
...
namespace Catch {
	...
	auto BenchmarkLooper::needsMoreIterations() -> bool
	{
		...
		// Exponentially increasing iterations until we're confident in our
		// timer resolution
...
namespace Catch {
	...
	bool FatalConditionHandler::isSet = false;
	struct sigaction
	  FatalConditionHandler::oldSigActions[sizeof(signalDefs) /
	                                       sizeof(SignalDefs)] = {};
	...
	namespace {
		  ...
		  , private NonCopyable
		{
		  ...
		  public: // IRegistryHub
			...
			IExceptionTranslatorRegistry& getExceptionTranslatorRegistry()
			  override
			{
...
namespace Catch {
	...
	namespace {
		...
		{
			auto reporter =
			  Catch::getRegistryHub().getReporterRegistry().create(reporterName,
			                                                       config);
...
#undef CATCH_CONFIG_DEFAULT_REPORTER
		...
		void addListeners(IStreamingReporterPtr& reporters,
		                  IConfigPtr const& config)
		{
...
namespace Catch {
	                    ...
	                    std::string const& withThis)
	{
		...
		while (i != std::string::npos) {
			replaced = true;
			str =
			  str.substr(0, i) + withThis + str.substr(i + replaceThis.size());
		...
		CATCH_ENFORCE(!isReservedTag(tag),
		              "Tag name: [" << tag << "] is not allowed.\n"
		                            << "Tag names starting with non "
		                               "alpha-numeric characters are reserved\n"
		                            << _lineInfo);
	}
...
namespace Catch {
	namespace TestCaseTracking {
		...
		{}
		bool TrackerBase::TrackerHasName::operator()(
		  ITrackerPtr const& tracker) const
		{
		...
		class AssertionPrinter
		{
		  ...
		  private:
			...
			void printRemainingMessages(Colour::Code colour = dimColour())
			{
				...
				// using messages.end() directly yields (or auto) compilation
				// error:
...
namespace Catch {
	...
	void MultipleReporters::benchmarkStarting(
	...
	void MultipleReporters::assertionStarting(
