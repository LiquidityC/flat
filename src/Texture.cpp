...
bool
Texture::loadFromFile(std::string path, SDL_Renderer* renderer)
{
	freeTexture();
...
bool
Texture::loadFont(std::string path, size_t size)
{
	freeFont();
...
bool
Texture::loadFromRenderedText(std::string text,
                              SDL_Color color,
...
double
Texture::getRotation() const
{
	return rotation;
}

void
Texture::setRotation(double angle, SDL_Point point)
{
	this->rotation = angle;
...
SDL_RendererFlip
Texture::getFlip() const
{
	return flip;
}

void
Texture::setFlip(SDL_RendererFlip flip)
{
	this->flip = flip;
}
} // namespace flat2d
