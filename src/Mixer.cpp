...
namespace flat2d {
	...
	bool Mixer::loadMusic(int id, std::string path)
	{
		...
		if (tune == nullptr) {
			std::cerr << "Unable to load music " << path << ": "
	...
	bool Mixer::playingMusic() { return Mix_PlayingMusic(); }
	...
	bool Mixer::pausedMusic() { return Mix_PausedMusic(); }
