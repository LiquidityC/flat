...
bool
Mixer::loadEffect(int id, std::string path)
{
...
bool
Mixer::loadMusic(int id, std::string path)
{
	...
	if (tune == nullptr) {
		std::cerr << "Unable to load music " << path << ": " << Mix_GetError()
		          << std::endl;
		return false;
	}
	music[id] = tune;
