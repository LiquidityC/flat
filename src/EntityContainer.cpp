...
void
EntityContainer::addLayer(unsigned int layer)
{
...
std::vector<int>
EntityContainer::getLayerKeys() const
{
	std::vector<int> keys;
...
void
EntityContainer::registerObject(Entity* object, Layer layer)
{
...
void
EntityContainer::repopulateCollidables()
{
...
EntityShape
EntityContainer::createBoundingBoxFor(const EntityProperties& props) const
{
	EntityShape vShape =
...
void
EntityContainer::registerObjectToSpatialPartitions(Entity* o)
{
...
void
EntityContainer::clearObjectFromCurrentPartitions(Entity* o)
{
...
void
EntityContainer::clearObjectFromUnattachedPartitions(Entity* o)
{
...
void
EntityContainer::addObjectToSpatialPartitionFor(Entity* o, int x, int y)
{
...
void
EntityContainer::setSpatialPartitionDimension(unsigned int i)
{
	spatialPartitionDimension = i;
}

void
EntityContainer::unregisterObject(Entity* object)
{
	std::string objId = object->getStringId();
	objects.erase(objId);
...
void
EntityContainer::reinitLayerMap()
{
...
bool
EntityContainer::isUninitiated(const std::string& id) const
{
...
void
EntityContainer::unregisterAllObjectsFor(Layer layer)
{
	if (layeredObjects.find(layer) == layeredObjects.end()) {
		return;
	}
...
void
EntityContainer::initiateEntities(const GameData* gameData)
{
#ifdef FPS_DBG
	TIME_FUNCTION;
#endif
	for (auto it = uninitiatedEntities.begin(); it != uninitiatedEntities.end();
	     it++) {
...
void
EntityContainer::handleObjects(const SDL_Event& event, const GameData* gameData)
{
#ifdef FPS_DBG
	TIME_FUNCTION;
#endif
...
void
EntityContainer::renderObjects(const GameData* data) const
{
#ifdef FPS_DBG
	TIME_FUNCTION;
#endif
...
void
EntityContainer::moveObjects(const GameData* data)
{
#ifdef FPS_DBG
	TIME_FUNCTION;
#endif
...
void
EntityContainer::handlePossibleObjectMovement(Entity* entity)
{
...
size_t
EntityContainer::getObjectCount() const
{
	return objects.size();
}

size_t
EntityContainer::getObjectCountFor(Layer layer)
{
	if (layeredObjects.find(layer) == layeredObjects.end()) {
		return 0;
	}
...
size_t
EntityContainer::getCollidablesCount() const
{
	return collidableObjects.size();
}

void
EntityContainer::clearDeadObjects()
{
...
size_t
EntityContainer::getSpatialPartitionCount() const
{
	return spatialPartitionMap.size();
}

void
EntityContainer::iterateAllMovingObjects(EntityIter func) const
{
	for (auto it = objects.begin(); it != objects.end(); it++) {
...
void
EntityContainer::iterateCollidablesFor(const Entity* source, EntityIter func)
{
...
Entity*
EntityContainer::checkAllObjects(EntityProcessor func) const
{
	for (auto it = objects.begin(); it != objects.end(); it++) {
		if (func(it->second)) {
			return it->second;
...
Entity*
EntityContainer::checkAllCollidableObjects(EntityProcessor func) const
{
...
Entity*
EntityContainer::checkCollidablesFor(const Entity* source, EntityProcessor func)
{
	const EntityProperties::Areas& currentAreas =
	  source->getEntityProperties().getCurrentAreas();
	for (auto areaIter = currentAreas.begin(); areaIter != currentAreas.end();
	     areaIter++) {
...
void
EntityContainer::iterateCollidablesIn(Layer layer, EntityIter func)
{
	if (layeredObjects.find(layer) == layeredObjects.end()) {
		return;
	}
