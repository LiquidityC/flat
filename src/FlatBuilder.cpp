...
namespace flat2d {
	...
	bool FlatBuilder::initSDL(std::string title,
	                          int screenWidth,
	                          int screenHeight)
	{
	...
	bool FlatBuilder::initContainers()
	{
		...
		gameData = new GameData(entityContainer,
		                        collisionDetector,
		                        mixer,
		                        renderData,
		                        deltatimeMonitor);
	...
	GameData* FlatBuilder::getGameData() const { return gameData; }

	GameEngine* FlatBuilder::getGameEngine() const { return gameEngine; }

	int FlatBuilder::loadSDL(const std::string& name,
	                         int fps,
	                         int screenWidth,
	                         int screenHeight)
	{
