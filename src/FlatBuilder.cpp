...
bool
FlatBuilder::initSDL(std::string title, int screenWidth, int screenHeight)
{
	...
	if (hidpi) {
		if (SDL_RenderSetLogicalSize(
		      window->getRenderer(), screenWidth, screenHeight)) {
			std::cerr << "Unable to change render logical size: "
	...
	if (!(IMG_Init(imgFlags) & imgFlags)) {
		std::cerr << "Unable to initialize SDL_image: " << IMG_GetError()
		          << std::endl;
		return false;
	}

	if (TTF_Init() == -1) {
		std::cerr << "Unable to initiate SDL2_ttf: " << TTF_GetError()
		          << std::endl;
		return false;
	}

	if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048) < 0) {
		std::cerr << "Could not initialize SDL_Mixer: " << Mix_GetError()
		          << std::endl;
		return false;
	}
...
GameData*
FlatBuilder::getGameData() const
{
	return gameData;
}

GameEngine*
FlatBuilder::getGameEngine() const
{
	return gameEngine;
}

int
FlatBuilder::loadSDL(const std::string& name,
                     int fps,
                     int screenWidth,
                     int screenHeight)
{
